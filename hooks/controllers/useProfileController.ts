...
export const useProfileController = (
  ...
  const handleRestoreBackup = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (activeUser.id === 'DEMO') { showToast("Restauração indisponível em Demo.", "error"); return; }
  ...
  const handleResetData = async () => {
      ...
      try {
          ...
          await supabase.from('perfis').update({ total_available_capital: 0, interest_balance: 0 }).eq('id', activeUser.id);
