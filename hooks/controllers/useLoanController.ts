...
export const useLoanController = (
  ...
  const handleSaveNote = async () => {
    ...
    if (activeUser.id === 'DEMO') {
      setLoans(loans.map(l =>
        l.id === ui.noteModalLoan.id ? { ...l, notes: ui.noteText } : l
      ));
   ...
   * Agora o botão ABRE o Portal do Cliente
   */
  const handleGenerateLink = (loan: Loan) => {
  const client = clients.find(c => c.id === loan.clientId);
  const accessCode =
    (client as any)?.access_code || (client as any)?.accessCode;

  let url = `${window.location.origin}/?portal=${loan.id}`;

  if (accessCode) url += `&code=${accessCode}`;

  // ✅ abre PIX automaticamente no portal
  url += `&pix=1`;
  ...
  const openConfirmation = (config: any) => {
    ...
    const shouldShowRefund =
      config.type === 'DELETE' || config.type === 'ARCHIVE';
  ...
  const executeConfirmation = async () => {
    ...
    try {
      if (ui.confirmation.type === 'REVERSE_TRANSACTION') {
        ...
        const targetId =
          typeof ui.confirmation.target === 'string'
            ? ui.confirmation.target
            : ui.confirmation.target.id;
        ...
        const msg = await ledgerService.executeLedgerAction({
          ...
          activeUser,
          sources,
          refundChecked: ui.confirmation.showRefundOption
            ? ui.refundChecked
            : false,
        });
