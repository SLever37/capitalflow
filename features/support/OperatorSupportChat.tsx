...
const ONLINE_WINDOW_SECONDS = 120; // 2 minutos (ajuste como quiser)
...
export const OperatorSupportChat = ({
  ...
  const statusInfo = useMemo(() => {
    ...
    // Mostra “Online” só se recente; senão “Ativo há X”
    const label = isOnline ? 'Online' : `Ativo há ${diffLabel(selectedChat.timestamp)}`;
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${
                      statusInfo.isOnline ? 'bg-emerald-500 animate-pulse' : 'bg-slate-500'
                    }`}
                  />
                  <p
                    className={`text-[9px] font-bold uppercase truncate ${
                      statusInfo.isOnline ? 'text-emerald-500' : 'text-slate-400'
                    }`}
                  >
                    {statusInfo.label} • Contrato #{selectedChat.loanId.slice(0, 4)}
                  </p>
                </div>
              </div>
            </div>
