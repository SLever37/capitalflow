...
export const OperatorSupportChat = ({ activeUser, onClose }: { activeUser: any, onClose: () => void }) => {
    ...
    return (
        <Modal onClose={onClose} title="Atendimento Profissional">
            <div className="flex flex-col h-[600px] -m-6 sm:-m-12 bg-slate-900">
                {!selectedChat ? (
                    <div className="p-6 space-y-3 overflow-y-auto custom-scrollbar">
                        <div className="bg-emerald-600/10 border border-emerald-500/20 p-4 rounded-2xl mb-4 flex items-center gap-3">
                            ...
                            <p className="text-[10px] text-emerald-300 font-bold uppercase tracking-widest leading-tight">Painel de Suporte Persistente: Histórico seguro no banco de dados.</p>
                        </div>
                        ...
                        {chats.length === 0 ? (
                            ...
                            <button 
                                ...
                                className="w-full p-5 bg-slate-950 border border-slate-800 rounded-3xl flex items-center justify-between hover:border-blue-500 transition-all group"
                            >
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 bg-slate-800 rounded-2xl flex items-center justify-center text-blue-500 font-black border border-slate-700">
                                        <User size={24}/>
                                    </div>
                                ...
                                {chat.unreadCount > 0 && (
                                    <span className="bg-rose-500 text-white text-[10px] font-black w-6 h-6 flex items-center justify-center rounded-full animate-bounce shadow-lg shadow-rose-900/40">
                                        {chat.unreadCount}
                                    </span>
                                )}
                            </button>
                    ...
                    <div className="flex flex-col h-full animate-in slide-in-from-right duration-300">
                        <div className="p-4 bg-slate-950 border-b border-slate-800 flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-emerald-500 font-bold uppercase">Conexão Criptografada</p>
                            </div>
                        </div>
                        <div className="flex-1 overflow-hidden">
                            <ChatContainer 
                                ...
                                senderType="OPERATOR"
                                placeholder="Responda ao seu cliente..."
                            />
                        </div>
                    </div>
                )}
