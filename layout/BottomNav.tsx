...
import { LayoutDashboard, Users, Wallet, LayoutGrid, Plus, Briefcase, PiggyBank, ChevronLeft, Calendar, Calculator, ArrowRightLeft, Megaphone, Scale } from 'lucide-react';
...
export const BottomNav: React.FC<BottomNavProps> = ({ 
  ...
  const getTabIcon = (tab: string, active: boolean) => {
    ...
    switch (tab) {
      ...
      case 'LEGAL': return <Scale size={size} className={active ? 'text-yellow-400' : ''}/>;
  ...
  const mobileTabs = ['DASHBOARD', 'AGENDA', 'SIMULATOR', 'PERSONAL_FINANCE', 'ACQUISITION', 'FLOW', 'CLIENTS', 'SOURCES', 'LEGAL'];
  if (!isStaff) {
  ...
  return (
    <div 
      ...
      className={`md:hidden fixed bottom-0 left-0 right-0 bg-slate-950/90 backdrop-blur-xl border-t border-slate-800 z-[100] flex items-center p-2 pb-safe overflow-x-auto hide-scrollbar gap-2 px-4 ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}`}
