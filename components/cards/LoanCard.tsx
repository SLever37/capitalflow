...
import {
  ...
  RefreshCcw, Settings, Clock, Calculator, Undo2, Lock
} from 'lucide-react';
import { Loan, LoanStatus, Installment, CapitalSource, LedgerEntry } from '../../types';
...
export const LoanCard: React.FC<LoanCardProps> = ({
  ...
  onUploadDoc, onViewPromissoria, onViewDoc, onReviewSignal, onOpenComprovante, onReverseTransaction, isStealthMode
}) => {
  const [showPaidLegacy, setShowPaidLegacy] = useState(false);
  ...
  // Verifica saldo zero geral (para qualquer modalidade)
  ...
  // CÁLCULOS ESPECÍFICOS PARA DAILY FIXED TERM
  ...
  // FINALIZADO = PAID status OU Saldo Zero OU Prazo Fixo completo
  const isFullyFinalized = isPaid || isZeroBalance || (isFixedTerm && fixedTermStats && fixedTermStats.paidDays >= fixedTermStats.totalDays);

  let cardStyle = "bg-slate-900 border-slate-800";
  let iconStyle = "bg-slate-800 text-slate-500";

  if (hasNotes) { cardStyle = "bg-amber-950/20 border-amber-500/30"; }
  
  if (isFullyFinalized) {
    // Estilo VERDE intenso para finalizados
    ...
    const daysUntilDue = Math.min(...loan.installments
      .filter(i => i.status !== LoanStatus.PAID)
      .map(i => -getDaysDiff(i.dueDate)));
  ...
  const orderedInstallments = React.useMemo(() => {
    ...
    if (showProgress) {
      if (!showPaidLegacy && !isPaid) {
        all = all.filter(i => i.status !== LoanStatus.PAID && Math.round(i.principalRemaining) > 0);
      }
    }
    return all;
  }, [loan.installments, showProgress, isPaid, showPaidLegacy]);

  // Renderizador de Item de Histórico com Auditoria
      ...
      <div key={t.id} className="flex flex-col border-b border-slate-800/50 pb-2 mb-2 last:border-0 last:pb-0 last:mb-0 group">
        <div className="flex justify-between items-center text-xs">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-white font-bold">{isAudit ? 'Edição Manual' : (t.notes || (t.type === 'LEND_MORE' ? 'Empréstimo' : 'Pagamento'))}</p>
          ...
          <div className="flex items-center gap-2">
            ...
            {isReversible && (
                <button 
                    onClick={(e) => { e.stopPropagation(); onReverseTransaction(t, loan); }}
  ...
  return (
    <div
      className={`border transition-all duration-300 rounded-[2rem] sm:rounded-[2.5rem] p-5 sm:p-8 hover:shadow-2xl cursor-pointer overflow-hidden active:scale-[0.99] ${cardStyle} ${isExpanded ? 'ring-1 ring-white/10' : ''} ${isCritical ? 'animate-pulse ring-1 ring-rose-500' : ''}`}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        <div className="flex items-center gap-4 sm:gap-6 w-full">
          <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center font-black text-lg sm:text-xl transition-all flex-shrink-0 ${iconStyle}`}>
            {isCritical ? <ShieldAlert size={24} /> : isFullyFinalized ? <CheckCircle2 size={24}/> : loan.debtorName[0]}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-2 mt-1 sm:mt-2">
              ...
              {(() => {
                const nextInst =
                  loan.installments.find(i => i.status !== LoanStatus.PAID) ||
        ...
        <div className="mt-8 sm:mt-10 space-y-6 sm:space-y-8 animate-in slide-in-from-top-4 duration-500" onClick={e => e.stopPropagation()}>
          ...
          <div className="bg-slate-950/50 p-5 rounded-3xl border border-slate-800">
            ...
            <div className="space-y-2 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
              {allLedger.length > 0 ? (
                allLedger.map(t => renderLedgerItem(t))
              ) : (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 items-stretch">
            {orderedInstallments.map((inst, i) => {
              ...
              if (inst.status === LoanStatus.PAID || isZeroBalance) { 
                  ...
                  // Status simplificado para Prazo Fixo
              ...
              // Habilita/Desabilita botão de ação
              const isActionDisabled = inst.status === LoanStatus.PAID || isFullyFinalized;

              return (
                <div
                  key={inst.id}
                  className={`p-4 sm:p-5 rounded-2xl sm:rounded-3xl border flex flex-col justify-between h-full ${inst.status === LoanStatus.PAID || isFixedTermDone || isZeroBalance
                ...
                >
                  {/* --- BLOCO VISUAL ESPECIAL: CONTADOR PARA DAILY_FIXED_TERM --- */}
                  {isFixedTerm && fixedTermStats ? (
                      <div className="mb-4">
                          ...
                          <div className="h-2 w-full bg-slate-800 rounded-full overflow-hidden mb-3">
                              <div className="h-full bg-blue-600 rounded-full transition-all duration-500" style={{width: `${fixedTermStats.progressPercent}%`}}></div>
                          </div>
                      ...
                      // BLOCO PADRÃO PARA OUTRAS MODALIDADES
                      <div className="flex justify-between items-start mb-3 sm:mb-4">
                        <div>
                          <div className="flex items-center gap-2 mb-4">
                            <p className="text-[8px] sm:text-[9px] font-black text-slate-500 uppercase">{showProgress ? `${realIndex}ª Parcela` : 'Detalhes'}</p>
                            {inst.renewalCount && inst.renewalCount > 0 ? (
                              <span
                                className="text-[8px] font-black bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded flex items-center gap-1"
                                title="Vezes que os juros foram pagos (Renovações)"
                              >
                                <RefreshCcw size={8} /> {inst.renewalCount}x
                              </span>
                            ) : null}
                          </div>
                          ...
                          <div className="relative pl-3 border-l-2 border-slate-800 space-y-4 my-2">
                            <div className="relative">
                              ...
                              <p className="text-[9px] font-bold text-slate-500 uppercase leading-none">
                                {strategy.card.dueDateLabel(inst, loan)}
                              </p>
                              <p className={`text-xs font-black ${isPrepaid ? 'text-emerald-400' : isLateInst && !isPaid ? 'text-rose-400' : 'text-white'}`}>
                                {formatBRDate(paidUntilDate)}
                              </p>
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="mb-4 sm:mb-5">
                    <div className="flex flex-col">
                      <div className="flex items-baseline gap-2 flex-wrap">
                        ...
                        {(debt.interest + debt.lateFee) > 0 && !isPrepaid && (
                          <>
                  ...
                  {!isActionDisabled ? (
                    <button
                      onClick={() => onPayment(loan, inst, debt)}
          ...
          <div className="flex flex-col gap-3 pt-4 border-t border-slate-800 sm:flex-row sm:items-center sm:justify-between">
            ...
            {/* BOTÃO DE EDITAR: Só aparece se NÃO estiver finalizado/arquivado */}
            {!loan.isArchived && !isFullyFinalized && (
